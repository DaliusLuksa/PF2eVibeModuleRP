...
/** Dying 4 means death per the rules, so higher starting values are capped there. */
...
export class DyingAutomationTool {
	...
	/** HP crossed from above zero to zero: apply the rules-correct starting Dying value. */
	static async _onKnockdown(actor) {
		...
		const startValue = Math.min((wasCritical ? 2 : 1) + woundedValue, DYING_DEATH_VALUE);
	...
	static async _applyConditionOp(actor, op, value) {
		...
		if (op === "applyDying") {
			...
			await actor.increaseCondition(DYING_SLUG, { value: Math.min(Number(value) || 1, DYING_DEATH_VALUE) });
			return true;
		}
		if (op === "removeDying") {
			const dying = actor.getCondition?.(DYING_SLUG);
